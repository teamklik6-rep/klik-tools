...
У кожній студії плаваюча кнопка **FX** (праворуч унизу) відкриває спільну панель пост-обробки: блюр, зерно (розмір/blend/анімація), віньєтка, RGB-аберація, сканлайни, кольорокорекція. Ефекти видно у превʼю, а кнопки «PNG з FX» / «WebM з FX» роблять чесний композит у файл. Налаштування зберігаються у `localStorage("klikFX")` і спільні для всіх студій (`tools/postfx.js`).
